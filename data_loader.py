...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        CREATE (p)-[t:TRIP {{distance:toFloat(row.trip_distance), fare:toFloat(row.fare_amount), pickup_dt:datetime(row.tpep_pickup_datetime), dropoff_dt:datetime(row.tpep_dropoff_datetime)}}]->(d)
        """
...
def main():
    ...
    while attempt < total_attempts:
        try:
            data_loader = DataLoader("neo4j://localhost:7687", "neo4j-standalone", "project2phase2")
